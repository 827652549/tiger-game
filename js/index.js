...
window.onload=()=>{
    $('#Marquee_x').jcMarquee({ 'marquee':'x','margin':'10px','speed':2 });
};
let getNumUpDown=(n)=> {
    return Math.floor(Math.random() * (n+1))
};
let getArrayUpDown=(maxNum, len)=> {
    var result = [];
    while (result.length < len) {
        var num = getNumUpDown(maxNum);
        if (result.length === (len - 1) && result.filter(item => item === num).length === (len - 1)) {
    ...
    let interval = setInterval(
        ()=>{
            ...
            if (secondNum<0){
                ...
                //延长点击时间，与动画保持一致
                setTimeout(()=>{document.getElementById('font-iknow').onclick=buttonIKnow},500);
